...
<table border="2">
    ...
    // get number of pages
    if($sumRecord % $numEachPage != 0){
        $numPages = $sumRecord / $numEachPage + 1;
    }else{
        $numPages = $sumRecord / $numEachPage;
    }
    $offset = ($currentPage-1) * $numEachPage;
    $query2 = "select * from tb_meeting order by meetingDate desc limit $offset,$numEachPage";
...
<?php
// BUG
    echo "<a href='showMeeting.php?currentPage=1'>First Page</a><br/>   ";
    if($currentPage > 1){
        $previousPage = $currentPage-1;
        echo "<a href='showMeeting.php?currentPage=".$previousPage."'>Previous Page</a><br/>   ";
    }
    if($currentPage < $numPages){
        $nextPage = $currentPage + 1;
        echo "<a href='showMeeting.php?currentPage=".$nextPage."'>Next Page</a><br/>   ";
    }
    echo "<a href='showMeeting.php?currentPage=".$numPages."'>Last Page</a><br/> ";
    html_footer();